...
export const FF: React.FC<{ label: string; span2?: boolean; required?: boolean; children: React.ReactNode }> = ({ label, span2, required, children }) => (
  <div style={{ gridColumn: span2 ? 'span 2' : 'span 1' }}>
    {/* An empty label still laid out a full line box plus its margin — about 21px of
        dead space above every repeated row (the invoice list labels only its first
        row). Skip the element entirely when there is nothing to show. */}
    {label && (
